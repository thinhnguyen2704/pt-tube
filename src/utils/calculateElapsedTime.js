const calculateElapsedTime = (publishedTime) => {
  ...
  // All time values are in milisecond(ms)
  if (elapsedTime >= 31557600000) {
    elapsedTime = Math.floor(elapsedTime / 31557600000).toLocaleString('en-US');
    return elapsedTime >= 2
      ? (elapsedTime += ' years ago')
      : (elapsedTime += ' year ago');
  } else if (elapsedTime >= 2629746000) {
    elapsedTime = Math.floor(elapsedTime / 2629746000).toLocaleString('en-US');
    return elapsedTime >= 2
      ? (elapsedTime += ' months ago')
      : (elapsedTime += ' month ago');
