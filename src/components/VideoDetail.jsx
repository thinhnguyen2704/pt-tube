...
const VideoDetail = () => {
  ...
  useEffect(() => {
    fetchFromAPI(`videos?part=snippet,statistics,contentDetails&id=${id}`).then(
      (data) => setVideoDetail(data.items[0])
    );

    fetchFromAPI(
      `commentThreads?part=snippet&videoId=${id}&maxResults=20`
    ).then((data) => setCommentThread(data.items));
            ...
            <Box sx={{ top: '86px' }}>
              ...
              >
                <Link to={`/ptube/channel/${channelId}`}>
                  <Typography
                    variant={{ sm: 'subtitle6', md: 'h6' }}
                    color='#fff'
                    fontWeight='bold'
                  >
                    {channelTitle}
                    <CheckCircle
                      sx={{ fontSize: '12px', color: '#606060', ml: '5px' }}
                    />
                  </Typography>
                </Link>
